...
```text
https://github.com/scoltzero/msm-free/releases/tag/v0.2.2
```

Linux x86_64 安装包：

```text
https://github.com/scoltzero/msm-free/releases/download/v0.2.2/msm-free-linux-amd64.tar.gz
```

Linux ARM64 安装包：

```text
https://github.com/scoltzero/msm-free/releases/download/v0.2.2/msm-free-linux-arm64.tar.gz
```
...
sudo /tmp/msm-free-0.2.2-linux-amd64/install.sh
```
...
sudo /tmp/msm-free-0.2.2-linux-arm64/install.sh
```
...
```bash
make package VERSION=0.2.2 GOOS=linux GOARCH=amd64
```

构建 Linux ARM64 压缩包：

```bash
make package VERSION=0.2.2 GOOS=linux GOARCH=arm64
```

构建 Unraid 插件产物：

```bash
make unraid VERSION=0.2.2 UNRAID_VERSION=0.2.2 GITHUB_REPO=scoltzero/msm-free RELEASE_TAG=v0.2.2
```
...
- `dist/unraid/msm-free-0.2.2-x86_64-1.txz`
